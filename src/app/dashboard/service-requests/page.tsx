...
import { CheckCircle2, Plus, RefreshCw, Search, Wrench, XCircle } from "lucide-react";
...
export default function ServiceRequestsPage() {
  ...
  const [itemForm, setItemForm] = useState({ requestId: "", serviceId: "", quantity: "1" });

  async function loadData(nextSearch = search) {
    setLoading(true);
    setError(null);
    try {
      const [customerResponse, facilityResponse, serviceResponse, requestResponse] =
        await Promise.all([
  ...
  async function runAction(id: string, action: "approve" | "reject" | "convert") {
    setActionId(id);
    setError(null);
    try {
      ...
      if (action === "convert") await operationsApi.convertServiceRequest(id, {});
      await loadData(search);
    } catch (err) {
      setError(getErrorMessage(err));
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Service Requests</h1>
          <p className="mt-1 text-sm text-slate-500">{total} requests in the active workspace.</p>
        </div>
        <form
        ...
        >
          <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search requests" icon={<Search className="h-4 w-4" />} />
            ...
            <table className="w-full min-w-[880px] text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {requests.map((request) => (
                  <tr key={request.id} className="hover:bg-slate-50" onClick={() => setItemForm((current) => ({ ...current, requestId: request.id }))}>
                    <td className="px-5 py-3.5"><p className="font-medium text-slate-900">{request.title}</p><p className="text-xs text-slate-400">{request.requestNumber}</p></td>
                    <td className="px-5 py-3.5 text-slate-600">{request.customer?.name || "Not set"}</td>
                    <td className="px-5 py-3.5 text-slate-600">{request.serviceLine.replaceAll("_", " ").toLowerCase()}</td>
                    <td className="px-5 py-3.5 text-slate-600">{request.priority.toLowerCase()}</td>
                    <td className="px-5 py-3.5 text-slate-600">{request._count?.items ?? 0}</td>
                    <td className="px-5 py-3.5"><StatusPill status={request.status} /></td>
                    <td className="px-5 py-3.5">
                      <div className="flex gap-2">
                        <Button type="button" size="sm" variant="ghost" disabled={actionId === request.id} onClick={() => void runAction(request.id, "approve")}><CheckCircle2 className="h-4 w-4" /></Button>
                        <Button type="button" size="sm" variant="ghost" disabled={actionId === request.id} onClick={() => void runAction(request.id, "reject")}><XCircle className="h-4 w-4" /></Button>
                        <Button type="button" size="sm" variant="outline" disabled={actionId === request.id} onClick={() => void runAction(request.id, "convert")}><Wrench className="h-4 w-4" /></Button>
                      </div>
                    </td>
                  </tr>
                ))}
                {!loading && requests.length === 0 && (
                  <tr><td className="px-5 py-8 text-center text-slate-500" colSpan={7}>No service requests found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <form className="space-y-4 p-5" onSubmit={createRequest}>
              ...
              <Input label="Requested start" type="datetime-local" value={form.requestedStartAt} onChange={(event) => setForm((current) => ({ ...current, requestedStartAt: event.target.value }))} />
              <Button type="submit" loading={saving} fullWidth disabled={!form.customerId}><Plus className="h-4 w-4" />Save request</Button>
            </form>
          </Card>
          ...
          <Card>
            ...
            <form className="space-y-4 p-5" onSubmit={addItem}>
              <select className={selectClass} value={itemForm.requestId} onChange={(event) => setItemForm((current) => ({ ...current, requestId: event.target.value }))}>
                {requests.map((request) => <option key={request.id} value={request.id}>{request.requestNumber} - {request.title}</option>)}
              </select>
              <select className={selectClass} value={itemForm.serviceId} onChange={(event) => setItemForm((current) => ({ ...current, serviceId: event.target.value }))}>
                {services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}
              </select>
              <Input label="Quantity" type="number" min="0" step="0.01" value={itemForm.quantity} onChange={(event) => setItemForm((current) => ({ ...current, quantity: event.target.value }))} required />
              <Button type="submit" loading={actionId === itemForm.requestId} fullWidth variant="secondary" disabled={!itemForm.requestId || !itemForm.serviceId}>Add item</Button>
            </form>
          </Card>
        </div>
      </div>
